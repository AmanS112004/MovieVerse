...
export default function SearchBar({ onSelectMovie, placeholder = 'Search movies, shows, anime...', large = false }: SearchBarProps) {
  ...
  return (
    <div className="relative w-full" style={{ zIndex: 50 }}>
      {/* Input */}
      <motion.div
        animate={{ boxShadow: isFocused ? '0 0 40px rgba(225,29,72,0.25)' : '0 0 0px rgba(225,29,72,0)' }}
        className={cn(
          'flex items-center gap-3 px-5 rounded-2xl border transition-all',
          'bg-[rgba(11,15,26,0.8)] backdrop-blur-xl',
          'border-white/10',
          isFocused && 'border-[#E11D48]/50',
          large ? 'py-5' : 'py-3'
        )}
      >
        <Search className={cn('text-white opacity-70 flex-shrink-0', large ? 'w-6 h-6' : 'w-5 h-5')} />
        <input
          ref={inputRef}
          value={query}
          ...
          className={cn(
            'flex-1 bg-transparent outline-none text-white font-medium',
                  ...
                  return (
                    <motion.div
                      ...
                      className={cn(
                        ...
                        activeIndex === idx ? 'bg-[#E11D48]/10' : 'hover:bg-white/5'
                      )}
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-0.5">
                          ...
                          <span className="text-xs px-1.5 py-0.5 rounded bg-white/5 text-gray-400 font-medium">{type}</span>
